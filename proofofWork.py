...
cached_scripts = [ "https://chatgpt.com/backend-api/sentinel/sdk.js"]
cached_dpl = None
cached_time = 0
...
async def get_dpl(service):
    ...
    cached_dpl = ""
    try:
        r =  await service.session.get(f"{service.host_url}/", headers=headers, timeout=5)
        r.raise_for_status()
...
def get_config(user_agent):
    ...
    config = [
        ...
        cached_dpl,
        random.choice(["en-US", "zh-CN"]),
        random.choice(["en-US,en,es", "en-US,en,es,zh-CN"]),
        random.randint(0,8),
...
def get_answer_token(seed, diff, config):
    start = time.time()
    answer, solved,i = generate_answer(seed, diff, config)
    end = time.time()
    print(f'diff: {diff}, time: {int((end - start) * 1e6) / 1e3}ms, solved: {solved} ,run_time: {i}, answer: gAAAAAB{answer[:100]}')
...
def generate_answer(seed, diff, config):
    ...
    static_config_part2 = (
                ',' + json.dumps(config[4:9], separators=(',', ':'), ensure_ascii=False)[1:-1] + ',').encode()
    ...
    for i in range(500000):
        ...
        if hash_value[:diff_len] <= target_diff:
            return base_encode.decode(), True,i

    return "wQ8Lk5FbGpA2NcR9dShT6gYjU7VxZ4D" + pybase64.b64encode(f'"{seed}"'.encode()).decode(), False,0


def get_requirements_token(config):
    require, solved = generate_answer(format(random.random()), "0fffff", config)
    print('gAAAAAC' + require)
    print(solved)
